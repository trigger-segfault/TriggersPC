...
namespace PokemonManager.PokemonStructures {
	public class GBAPokemon : IPokemon {
		...
		private GBAPokemon invalidBackup;
		...
		public GBAPokemon(byte[] data, bool decrypted = false) {
		...
		public ushort DexID {
			get { return (ushort)(PokemonData != null ? PokemonData.DexID : 0); }
		...
		public ushort HP {
			get { return LittleEndian.ToUInt16(raw, 88); }
			set { LittleEndian.WriteUInt16(value, raw, 88); }
		}
		...
		public byte Move4PPUpsUsed {
			get { return ByteHelper.BitsToByte(raw, growthSubDataOffset + 8, 6, 2); }
			set {
				if (GameSave != null)
					GameSave.IsChanged = true;
		...
		public ushort CurrentHP {
			get { return LittleEndian.ToUInt16(raw, 86); }
			set { LittleEndian.WriteUInt16(value, raw, 86); }
		}
		public StatusConditionFlags StatusCondition {
			get { return (StatusConditionFlags)LittleEndian.ToUInt32(raw, 80); }
			set { LittleEndian.WriteUInt32((uint)value, raw, 80); }
		}
		...
		public void SetHiddenPowerType(PokemonTypes type) {
			...
			byte typeBits = (byte)(((int)type - 2) * 63 / 15); // The minimum value for this move type
			...
			bool reachedGoal = GetHiddenPowerType(typeBits) == type;
			if (!reachedGoal)
				minDeviation = -1;
			while ((typeBits + 1 <= 0x3F && GetHiddenPowerType((byte)(typeBits + 1)) == type) || !reachedGoal) {
				typeBits++;
				reachedGoal = GetHiddenPowerType(typeBits) == type;
				int newDeviation = ByteHelper.GetBitDeviation(ByteHelper.GetBits(typeBits), originalTypeBits);
				if ((minDeviation == -1 || newDeviation < minDeviation) && reachedGoal) {
			...
			if (reachedGoal)
				SetHiddenPowerTypeBits(ByteHelper.GetBits(minDeviationTypeBits));
		}
		public void SetHiddenPowerDamage(byte damage) {
			...
			bool reachedGoal = GetHiddenPowerDamage(damageBits) == damage;
			if (!reachedGoal)
				minDeviation = -1;
			while ((damageBits + 1 <= 0x3F && GetHiddenPowerDamage((byte)(damageBits + 1)) == damage) || !reachedGoal) {
				damageBits++;
				reachedGoal = GetHiddenPowerDamage(damageBits) == damage;
			...
			if (reachedGoal)
				SetHiddenPowerDamageBits(ByteHelper.GetBits(minDeviationDamageBits));
		}
		...
		public IPokemon Clone() {
			return CreateGBAPokemon(gameType, false);
		}
		...
		public static GBAPokemon CreateInvalidPokemon(GBAPokemon invalidBackup, GameTypes gameType = GameTypes.Any) {
			GBAPokemon pkm = new GBAPokemon();
