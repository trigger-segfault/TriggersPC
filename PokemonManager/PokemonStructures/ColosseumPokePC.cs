...
namespace PokemonManager.PokemonStructures {
	public class ColosseumPokePC : IPokePC {
		...
		private ColosseumPokeBox[] boxes;
		private ColosseumPokeParty party;
		private ColosseumDaycare daycare;
		...
		public bool HasRoomForPokemon(int amount = 1) {
			if (party != null)
				amount -= (int)(party.NumSlots - party.NumPokemon);
		...
		public void AddParty(byte[] data) {
			this.party = new ColosseumPokeParty(this, data);
		}
		public void AddDaycare(byte[] data) {
			this.daycare = new ColosseumDaycare(this, data);
		}
		...
		public void Load(byte[] data) {
			this.raw = data;
			for (int i = 0; i < 3; i++) {
				boxes[i] = new ColosseumPokeBox(this, (byte)i, ByteHelper.SubByteArray(9380 * i, data, 9380));
			}
		}
		public byte[] GetFinalData() {
			for (int i = 0; i < 3; i++) {
				ByteHelper.ReplaceBytes(raw, i * 9380, boxes[i].GetFinalData());
			}
			return raw;
		}
